...
import {
    ...
    Layout, Calendar, BookOpen, Clock, Zap, Brain,
    Linkedin, Globe, Rocket
} from 'lucide-react';
...
function FeatureCard({ title, desc, icon: Icon, tags, gradient, delay = 0, colSpan = '' }) {
    ...
    return (
        ...
        >20 }}
            ...
            className={`rounded-2xl border border-slate-200 bg-white hover:shadow-md transition-all p-6 flex flex-col ${colSpan}`}
        >
            <div className="mb-4 w-10 h-10 rounded-lg bg-cyan-100 flex items-center justify-center">
            ...
            {tags && (
                <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                    {tags.slice(0, 2).map((tag, i) => (
                        <span key={i} className="text-xs font-medium px-2 py-1 rounded-full bg-slate-100 text-slate-700">
                            {tag}
                        </span>
                    ))}
                </div>
    ...
    return (
        <div className="min-h-screen bg-[#050507] text-white font-sans selection:bg-cyan-500/30 selection:text-cyan-200 overflow-x-hidden">
            ...
            <nav className="fixed top-0 w-full z-50 border-b border-white/[0.04] bg-[#050507]/80 backdrop-blur-2xl">
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-6 text-sm font-medium text-gray-500">
                        <a href="#featurwhite text-slate-900 font-sans overflow-x-hidden">
            {/* Simple gradient background */}
            <div className="fixed inset-0 bg-gradient-to-b from-slate-50 to-white pointer-events-none z-0" /       <button
                            onClick={() => navigate('/auth')}
                            className="bg-white text-g-white/95 backdrop-blur-sm border-b border-slate-200">
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate('/')}>
                        <img src="/assets/logo.svg" alt="Mithra" className="w-7 h-7 rounded" />
                        <span className="font-bold text-sm tracking-tight text-slate-900">
                            Mithra <span className="text-cyan-600">Life OS</span>
                        </span>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-8 text-sm font-medium text-slate-600">
                        ...
                        <button
                            onClick={() => navigate('/auth')}
                            className="bg-cyan-600 text-white px-5 py-2 rounded-lg hover:bg-cyan-700 transition-all font-semibold text-xs
            ...
            <main ref={heroRef} className="pt-28 md:pt-36 pb-20 px-6 relative z-10">
                <motion.divslate-600" onClick={() => setIsMenuOpen(!isMenuOpen)}>
                ...
                {isMenuOpen && (
                    <div className="md:hidden bg-white border-t border-slate-200 px-6 py-4 space-y-3">
                        <a href="#features" onClick={() => setIsMenuOpen(false)} className="block text-slate-600 hover:text-slate-900 py-2">Features</a>
                        <a href="#how-it-works" onClick={() => setIsMenuOpen(false)} className="block text-slate-600 hover:text-slate-900 py-2">How It Works</a>
                        <a href="#about" onClick={() => setIsMenuOpen(false)} className="block text-slate-600 hover:text-slate-900 py-2">About</a>
                        <button onClick={() => { setIsMenuOpen(false); navigate('/auth'); }} className="w-full bg-cyan-600 text-white py-2 rounded-lg font-semibold text-sm">Get Started</button>
                    </div>
                )}tion={{ delay: 0.1, duration: 0.8, ease: luxe }}
